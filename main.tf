...
resource "oci_core_instance" "mapped" {
  for_each            = var.nodes_description
  display_name        = each.key
  availability_domain = data.oci_identity_availability_domains._.availability_domains[each.value["ad"]].name
  compartment_id      = local.compartment_id
  shape               = var.shape
  fault_domain        = format("FAULT-DOMAIN-%d", each.value["fd"])
  shape_config {
    memory_in_gbs = var.memory_in_gbs_per_node
    ocpus         = var.ocpus_per_node
  }
